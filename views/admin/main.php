...
<div><?php // echo anchor('admin/'.$this->config->item('module_name').'/create_event/', '+ Create New Event', 'title="Create New Event" '); ?></div><br />
...
<section class="item">
	...
	?>
	    <tr>
	        ...
	        <td align="center">
	            <?php echo anchor(site_url('admin/'.$this->config->item('module_name').'/edit_event/'. WsEncrypt( $event->id )),'Edit',array('class' => 'edit btn green')) ?>&nbsp;<a href="javascript: void(0);" <?php echo '" onclick="javascript:if(confirm(\'Are you sure you want to delete this event.\')){ document.location.href = \''.site_url('admin/'.$this->config->item('module_name').'/delete_event/'. WsEncrypt( $event->id )).'\';}else{return false;}" ' ?> title="Delete Test" class="confirm btn red delete">Delete</a>&nbsp;<?php echo anchor(site_url('admin/'.$this->config->item('module_name').'/update_event_status/'. WsEncrypt( $event->id )),$status_link_label,array('class' => 'btn orange edit')) ?>
	        </td>
	    </tr>
